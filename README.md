...
В проекте есть задачи, связанные с Instagram API, поэтому нам важно сразу посмотреть как вы с ними работаете. В самом проекте нет постов и комментариев, но это простое API, которое позволит вам выполнить тестовое задание, фокусируясь на разработке, а не на сложной логике самого API (например, сохранение сообщений).
